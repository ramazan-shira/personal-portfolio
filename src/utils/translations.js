export const translations = {
  en: {
    nav: {
      home: "Home",
      about: "About",
      skills: "Skills",
      services: "Services",
      experience: "Experience",
      projects: "Projects",
      contact: "Contact",
    },
    hero: {
      title: "Ramazan Shira",
      subtitle: "Full Stack Developer",
    },
    about: {
      title: "About Me",
      description1:
        "I am a Software Engineer with a passion for building robust, scalable, and user-focused solutions that bridge functionality and experience. With a solid foundation in full stack development and a strong understanding of software architecture, I bring a thoughtful approach to solving complex problems and delivering high-quality applications.",
      description2:
        "I thrive in collaborative environments, enjoy working across teams, and continuously seek to grow my technical and strategic capabilities. Committed to writing clean, maintainable code and contributing to meaningful projects, I aim to drive innovation and impact through technology.",
      highlights: [
        "Full Stack Development",
        "Software Architecture",
        "Scalable Solutions",
        "Clean Code Practices",
        "Team Leadership",
        "Problem Solving",
      ],
      getInTouch: "Get In Touch",
      viewProjects: "View Projects",
      downloadCV: "Download CV",
    },
    skills: {
      title: "Skills & Expertise",
      frontend: "Frontend",
      backend: "Backend",
      database: "Database",
      tools: "Tools & Methods",
      development: "Development",
      softSkills: "Soft Skills",
      skills: {
        html: "HTML",
        css: "CSS",
        javascript: "JavaScript",
        react: "React",
        responsiveDesign: "Responsive Design",
        nodejs: "Node.js",
        php: "PHP",
        java: "Java",
        restApis: "REST API",
        serverManagement: "Server Management",
        mysql: "MySQL",
        mongodb: "MongoDB",
        databaseDesign: "Database Design",
        queryOptimization: "Query Optimization",
        git: "Git",
        agile: "Agile",
        dataStructures: "Data Structures",
        algorithms: "Algorithms",
        networking: "Networking",
        versionControl: "Version Control",
        codeReview: "Code Review",
        testing: "Testing",
        cicd: "CI/CD",
        problemSolving: "Problem Solving",
        teamwork: "Teamwork",
        timeManagement: "Time Management",
        adaptability: "Adaptability",
      },
    },
    services: {
      title: "Services",
      subtitle: "Comprehensive development services tailored to your needs",
      fullStack: "Full Stack Development",
      fullStackDesc:
        "End-to-end application development from database design to interactive user interfaces, ensuring seamless integration and optimal performance.",
      frontend: "Frontend Development",
      frontendDesc:
        "Creating responsive, user-friendly interfaces with modern frameworks like React. Focus on performance, accessibility, and cross-browser compatibility.",
      backend: "Backend Development",
      backendDesc:
        "Building robust server-side applications with Node.js, PHP, and Java. Implementing secure APIs and efficient data processing systems.",
      database: "Database Management",
      databaseDesc:
        "Designing and optimizing database schemas for MySQL and MongoDB. Ensuring data integrity, security, and efficient query performance.",
      responsive: "Responsive Design",
      responsiveDesc:
        "Crafting seamless user experiences across all devices and screen sizes. Implementing modern design patterns and best practices.",
      architecture: "System Architecture",
      architectureDesc:
        "Planning and implementing scalable software architectures. Server infrastructure management and system optimization.",
    },
    experience: {
      title: "Work Experience",
      subtitle: "Professional journey and career highlights",
      jobs: [
        {
          company: "General Directorate of Archives",
          position: "Head of Digitization Sector & Full Stack Developer",
          period: "2024 - Present",
          responsibilities: [
            "Planned, coordinated, and oversaw all digitization initiatives",
            "Managed team operations and performance oversight",
            "Designed and implemented end-to-end full-stack applications",
            "Built and optimized interactive web interfaces and integrated systems",
          ],
        },
        {
          company: "Awebrie",
          position: "Frontend Developer",
          period: "2023 - 2024",
          responsibilities: [
            "Collaborated with designers to develop user-facing features",
            "Ensured cross-browser compatibility and optimized performance",
            "Implemented responsive designs for seamless user experiences",
            "Wrote clean, maintainable code and conducted code reviews",
          ],
        },
        {
          company: "Embedded Systems Design & Production",
          position: "IT Support Specialist",
          period: "2021 - 2023",
          responsibilities: [
            "Reviewed diagnostics and assessed software functionality",
            "Provided helpdesk IT support for end users and servers",
            "Set up user accounts and managed password issues",
            "Diagnosed and resolved technical hardware and software problems",
          ],
        },
      ],
    },
    projects: {
      title: "Projects",
      subtitle: "Showcase of my recent work and personal projects",
      viewCode: "View Code",
      liveDemo: "Live Demo",
      items: [
        {
          title: "Arkiva",
          description: "Digital archive platform for Albanian records.",
        },
        {
          title: "Fortis",
          description: "Employee training platform for Fortis.",
        },
        { title: "TaskEase", description: "Task management app for teams." },
        {
          title: "Notes App",
          description: "Real-time note taking application.",
        },
        {
          title: "Car Sales",
          description: "Car sales platform using React Redux.",
        },
        {
          title: "Pet Expo",
          description: "Event management platform for pet expo.",
        },
        {
          title: "User Management",
          description: "User management system with React.",
        },
        {
          title: "ToDo List",
          description: "Task organizer built with React Redux.",
        },
        {
          title: "Github Profile",
          description: "Github user card application.",
        },
        {
          title: "React Router Movies",
          description: "Movie catalog app using React Router.",
        },
        {
          title: "Digital Mastery",
          description: "Landing page built with HTML, CSS, JavaScript.",
        },
        {
          title: "Taste It",
          description: "Restaurant website built with HTML, CSS, JavaScript.",
        },
        {
          title: "Roll Dice",
          description: "Dice game built with HTML, CSS, JavaScript.",
        },
        {
          title: "Guess Number",
          description: "Number guessing game with HTML, CSS, JavaScript.",
        },
      ],
      technologies: {
        React: "React",
        "Node.js": "Node.js",
        MongoDB: "MongoDB",
        "REST API": "REST API",
        Java: "Java",
        MySQL: "MySQL",
        "Node JS": "Node JS",
        WebSocket: "WebSocket",
        "React Redux": "React Redux",
        HTML: "HTML",
        CSS: "CSS",
        JavaScript: "JavaScript",
      },
    },
    contact: {
      title: "Get In Touch",
      subtitle:
        "Have a project in mind? Let's work together to create something amazing",
      contactInfo: "Contact Information",
      email: "Email",
      phone: "Phone",
      location: "Location",
      connect: "Let's Connect!",
      connectMsg:
        "I'm always open to discussing new projects, creative ideas, or opportunities to be part of your vision.",
      name: "Name",
      subject: "Subject",
      message: "Message",
      namePlaceholder: "Your name",
      emailPlaceholder: "your.email@example.com",
      subjectPlaceholder: "Project inquiry",
      messagePlaceholder: "Tell me about your project...",
      sendMessage: "Send Message",
    },
    footer: {
      about:
        "Full Stack Developer passionate about building robust, scalable solutions and solving complex problems.",
      quickLinks: "Quick Links",
      madeBy: "Made by",
      connect: "Connect",
      copyright: "All rights reserved.",
    },
  },
  al: {
    nav: {
      home: "Kryefaqja",
      about: "Rreth Meje",
      skills: "Aftësitë",
      services: "Shërbime",
      experience: "Eksperienca",
      projects: "Projekte",
      contact: "Kontakt",
    },
    hero: {
      title: "Ramazan Shira",
      subtitle: "Zhvillues Full Stack",
    },
    about: {
      title: "Rreth Meje",
      description1:
        "Jam një Inxhinier Softuerësh me pasion për të krijuar zgjidhje të qëndrueshme, të shkallëzueshme dhe të fokusuar tek përdoruesi.",
      description2:
        "Përfitoj më së miri në mjedise bashkëpunuese, duke punuar nëpër ekipe dhe duke zhvilluar aftësitë teknike dhe strategjike.",
      highlights: [
        "Zhvillim Full Stack",
        "Arkitekturë Softueri",
        "Zgjidhje të Shkallëzuara",
        "Kod i Pastër",
        "Udhëheqje Ekipi",
        "Zgjidhje Probleme",
      ],
      getInTouch: "Kontakto",
      viewProjects: "Shiko Projekte",
      downloadCV: "Shkarko CV",
    },
    skills: {
      title: "Aftësi dhe Ekspertizë",
      frontend: "Frontend",
      backend: "Backend",
      database: "Baza e të Dhënave",
      tools: "Mjete & Metoda",
      development: "Zhvillimi",
      softSkills: "Aftësi Ndërpersonale",
      skills: {
        html: "HTML",
        css: "CSS",
        javascript: "JavaScript",
        react: "React",
        responsiveDesign: "Dizajn responsiv",
        nodejs: "Node.js",
        php: "PHP",
        java: "Java",
        restApis: "REST API",
        serverManagement: "Menaxhimi i Serverave",
        mysql: "MySQL",
        mongodb: "MongoDB",
        databaseDesign: "Dizajn i Bazës së të Dhënave",
        queryOptimization: "Optimizimi i Kërkesave",
        git: "Git",
        agile: "Agile",
        dataStructures: "Struktura të të Dhënave",
        algorithms: "Algoritme",
        networking: "Administrimi i Rrjetit",
        versionControl: "Version Control",
        codeReview: "Rishikim Kodit",
        testing: "Testim",
        cicd: "CI/CD",
        problemSolving: "Zgjidhje Problemesh",
        teamwork: "Punë në Ekip",
        timeManagement: "Menaxhimi i Kohës",
        adaptability: "Përshtatshmëria",
      },
    },
    services: {
      title: "Shërbime",
      subtitle: "Shërbime zhvillimi të personalizuara për nevojat tuaja",
      fullStack: "Zhvillim Full Stack",
      fullStackDesc:
        "Zhvillim aplikacionesh nga dizajni i bazës së të dhënave deri te ndërfaqet interaktive, me performancë optimale.",
      frontend: "Zhvillim Frontend",
      frontendDesc:
        "Krijimi i ndërfaqeve të përdoruesit me React. Fokus në performancë, aksesueshmëri dhe kompatibilitet ndër-shfletues.",
      backend: "Zhvillim Backend",
      backendDesc:
        "Zhvillimi i aplikacioneve server-side me Node.js, PHP, dhe Java. Implementim API të sigurta dhe procese efikase të të dhënave.",
      database: "Menaxhim i Bazës së të Dhënave",
      databaseDesc:
        "Dizajnimi dhe optimizimi i skemave të bazës së të dhënave për MySQL dhe MongoDB.",
      responsive: "Dizajn Responsiv",
      responsiveDesc:
        "Krijimi i eksperiencave të përdoruesit për të gjitha pajisjet dhe madhësitë e ekranit.",
      architecture: "Arkitekturë Sistemi",
      architectureDesc:
        "Planifikim dhe implementim arkitekturash të shkallëzuara softuerike.",
    },
    experience: {
      title: "Eksperienca",
      subtitle: "Udhëtimi profesional dhe pikat kryesore të karrierës",
      jobs: [
        {
          company: "Drejtoria e Përgjithshme e Arkivave",
          position:
            "Përgjegjës i Sektorit të Digjitalizimit & Zhvillues Full Stack",
          period: "2024 - Tani",
          responsibilities: [
            "Planifikimi, koordinimi dhe mbikëqyrja e të gjitha iniciativave të digjitalizimit",
            "Menaxhimi i operacioneve të ekipit dhe performancës",
            "Dizajnimi dhe implementimi i aplikacioneve full-stack",
            "Ndërtimi dhe optimizimi i ndërfaqeve web dhe sistemeve të integruara",
          ],
        },
        {
          company: "Awebrie",
          position: "Zhvillues Frontend",
          period: "2023 - 2024",
          responsibilities: [
            "Bashkëpunim me dizajnerët për zhvillimin e funksionaliteteve",
            "Sigurimi i kompatibilitetit ndër-shfletues dhe optimizimi i performancës",
            "Implementimi i dizajneve responsiv për përdoruesit",
            "Shkrimi i kodit të pastër dhe rishikimi i tij",
          ],
        },
        {
          company: "Embedded Systems Design & Production",
          position: "Specialist IT",
          period: "2021 - 2023",
          responsibilities: [
            "Rishikimi i diagnostikimeve dhe vlerësimi i funksionalitetit të softuerit",
            "Sigurimi i mbështetjes IT për përdoruesit dhe serverat",
            "Konfigurimi i llogarive dhe menaxhimi i fjalëkalimeve",
            "Zgjidhja e problemeve teknike hardware dhe software",
          ],
        },
      ],
    },
    projects: {
      title: "Projekte",
      subtitle: "Shfaqja e projekteve të mia të fundit",
      viewCode: "Shiko Kodin",
      liveDemo: "Demo Live",
      items: [
        {
          title: "Arkiva",
          description: "Platformë digjitale për arkivat shqiptare.",
        },
        {
          title: "Fortis",
          description: "Platformë trajnimi për punonjësit e Fortis.",
        },
        {
          title: "TaskEase",
          description: "Aplikacion për menaxhimin e detyrave në ekipe.",
        },
        {
          title: "Notes App",
          description: "Aplikacion për shënime në kohë reale.",
        },
        {
          title: "Car Sales",
          description: "Platformë shitjesh makinash me React Redux.",
        },
        {
          title: "Pet Expo",
          description: "Menaxhimi i eventeve për ekspozitën e kafshëve.",
        },
        {
          title: "User Management",
          description: "Sistem menaxhimi përdoruesish me React.",
        },
        {
          title: "ToDo List",
          description: "Organizues detyrash me React Redux.",
        },
        {
          title: "Github Profile",
          description: "Aplikacion kartelë përdoruesi Github.",
        },
        {
          title: "React Router Movies",
          description: "Aplikacion katalog filmash me React Router.",
        },
        {
          title: "Digital Mastery",
          description: "Landing page me HTML, CSS, JavaScript.",
        },
        {
          title: "Taste It",
          description: "Faqe restoranti me HTML, CSS, JavaScript.",
        },
        {
          title: "Roll Dice",
          description: "Loja me zar me HTML, CSS, JavaScript.",
        },
        {
          title: "Guess Number",
          description: "Loja e gjykimit të numrit me HTML, CSS, JavaScript.",
        },
      ],
      technologies: {
        React: "React",
        "Node.js": "Node.js",
        MongoDB: "MongoDB",
        "REST API": "REST API",
        Java: "Java",
        MySQL: "MySQL",
        "Node JS": "Node JS",
        WebSocket: "WebSocket",
        "React Redux": "React Redux",
        HTML: "HTML",
        CSS: "CSS",
        JavaScript: "JavaScript",
      },
    },
    contact: {
      title: "Kontakt",
      subtitle:
        "Ke një projekt në mendje? Le të punojmë së bashku për të krijuar diçka të mahnitshme",
      contactInfo: "Informacion Kontakti",
      email: "Email",
      phone: "Telefon",
      location: "Vendndodhja",
      connect: "Keni ide për projekt?",
      connectMsg:
        "Jam gjithmonë i hapur për të diskutuar projekte të reja, ide kreative ose mundësi për të qenë pjesë e vizionit tuaj.",
      name: "Emri",
      subject: "Subjekti",
      message: "Mesazhi",
      namePlaceholder: "Emri juaj",
      emailPlaceholder: "emaili.juaj@example.com",
      subjectPlaceholder: "Pyetje për projektin",
      messagePlaceholder: "Tregoni për projektin tuaj...",
      sendMessage: "Dërgo Mesazhin",
    },
    footer: {
      about:
        "Zhvillues Full Stack me pasion për të krijuar zgjidhje të shkallëzuara dhe për të zgjidhur probleme komplekse.",
      quickLinks: "Lidhje të Shpejta",
      connect: "Kontakto",
      madeBy: "Krijuar nga",
      copyright: "Të gjitha të drejtat e rezervuara.",
    },
  },
};

export function t(key, lang) {
  const keys = key.split(".");
  let value = translations[lang];
  for (const k of keys) {
    if (value === undefined) return key;
    value = value[k];
  }
  return value || key;
}
